...
/// struct for typed errors of method [`network_eras_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NetworkErasGetError {
...
/// struct for typed errors of method [`network_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NetworkGetError {
    Status400(models::Get400Response),
    Status403(models::Get403Response),
    Status404(models::BlocksLatestGet404Response),
    Status418(models::Get418Response),
...
pub async fn network_eras_get(configuration: &configuration::Configuration, ) -> Result<Vec<models::NetworkErasInner>, Error<NetworkErasGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/network/eras", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<NetworkErasGetError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn network_get(configuration: &configuration::Configuration, ) -> Result<models::Network, Error<NetworkGetError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<NetworkGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
