...
/// struct for typed errors of method [`ipfs_add`]
...
pub async fn ipfs_add(configuration: &configuration::Configuration, file: Option<std::path::PathBuf>) -> Result<models::IpfsAdd200Response, Error<IpfsAddError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/ipfs/add", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<IpfsAddError> = serde_json::from_str(&local_var_content).ok();
