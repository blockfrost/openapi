...
/// struct for typed errors of method [`ipfs_gateway_ipfs_path_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum IpfsGatewayIpfsPathGetError {
...
pub async fn ipfs_gateway_ipfs_path_get(configuration: &configuration::Configuration, ipfs_path: &str) -> Result<std::path::PathBuf, Error<IpfsGatewayIpfsPathGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/ipfs/gateway/{IPFS_path}", local_var_configuration.base_path, IPFS_path=crate::apis::urlencode(ipfs_path));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<IpfsGatewayIpfsPathGetError> = serde_json::from_str(&local_var_content).ok();
