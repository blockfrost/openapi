...
/// struct for typed errors of method [`metrics_endpoints_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MetricsEndpointsGetError {
...
pub async fn metrics_endpoints_get(configuration: &configuration::Configuration, ) -> Result<Vec<models::MetricsEndpointsInner>, Error<MetricsEndpointsGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/metrics/endpoints", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<MetricsEndpointsGetError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn metrics_get(configuration: &configuration::Configuration, ) -> Result<Vec<models::MetricsInner>, Error<MetricsGetError>> {
    let local_var_configuration = configuration;
